...
地图数据分为轻量概览 `public/data/terra_layout.json` 和 `public/data/terra-cities/<国家>/` 下提前生成的 gzip 城市详情。默认二维地图不加载 Region 内部道路与建筑；放大到地块层级后，页面根据视口中心判断当前城市，只请求并解压该城市详情。已载入城市会保留在内存缓存中，鼠标移到界面按钮不会卸载建筑。
...
若本地仍是旧版 `terra-nations` 国家详情，可一次性生成城市压缩文件并更新概览索引：

```bash
npm run data:cities -- public/data
```
